...
/** 将树形部门展平 */
export function flattenDepartments(tree: Department[]): DeptNode[] {
...
/** 从完整部门树中提取指定根节点的子树 */
export function extractDepartmentSubtree(rootId: string, tree: Department[]): Department[] {
  const findNode = (nodes: Department[]): Department | undefined => {
    for (const node of nodes) {
