import type { PersonOption } from '@/components/Common/Filter/types';
import {
...
export function toDepartmentPathMaps(pathMap?: Map<string, string>): DepartmentPathMaps | undefined {
...
export function resolvePersonDepartmentName(
  ...
  if (maps) {
    ...
    // 无 departmentId 时，用唯一叶子名匹配（如 信息部 → 技术部/信息部）
    if (departmentName) {
