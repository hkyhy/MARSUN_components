  ...
  const handleOk = async () => {
    try {
      ...
      try {
        ...
        await fileApi.reupload(record.id, formData);
      ...
      {record && (
        <Form form={form} layout="vertical">
          <Form.Item label="当前文件">
            <span className={classNames('reupload-modal-file-name-strong', styles['reupload-modal-file-name-strong'])}>{record.name}</span>
          </Form.Item>
          <Form.Item
            name="name"
            label="文件名"
