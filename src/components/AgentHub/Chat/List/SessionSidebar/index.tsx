import { VirtualScrollbar } from '@/components/Common/VirtualScrollbar';
import { Plus, Trash2, Undo2 } from '@/icons';
...
const SessionSidebar: React.FC<SessionSidebarProps> = ({
  ...
  <div className={classNames('session-sidebar-status', styles['session-sidebar-status'])}>
    <div className={classNames('session-sidebar-info', styles['session-sidebar-info'])}>
      <Text strong className={classNames('session-sidebar-detail', styles['session-sidebar-detail'])} title={title}>
        {title}
      </Text>
      <Tooltip title="新会话">
        <Button
    ...
    <VirtualScrollbar wrapperClassName={classNames('session-sidebar-media', styles['session-sidebar-media'])} className={classNames('session-sidebar-figure', styles['session-sidebar-figure'])}>
      {loading ? (
        ...
        <div className={classNames('session-sidebar-divider', styles['session-sidebar-divider'])}>
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无历史对话" className={classNames('session-sidebar-block1', styles['session-sidebar-block1'])} />
        </div>
      ) : (
        <div className={classNames('session-sidebar-block2', styles['session-sidebar-block2'])}>
          {sessions.map((session) => {
            ...
            return (
              <div
                key={session.id}
                className={classNames('session-sidebar-session-item', styles['session-sidebar-session-item'],
              ...
              >
                <button
                  ...
                  title={displayName}
                  className={classNames('session-sidebar-session-button', styles['session-sidebar-session-button'],
                ...
                <span className={classNames('session-sidebar-block3', styles['session-sidebar-block3'])}>
                  {timeLabel}
                </span>

                <div className={classNames('session-sidebar-block4', styles['session-sidebar-block4'])}>
                  <Popconfirm
                  ...
                  >
                    <Tooltip title={active ? '清空' : '删除'}>
                      <button
                        ...
                        className={classNames('session-sidebar-session-action-btn', styles['session-sidebar-session-action-btn'],
                      ...
                      >
                        {active ? <Undo2 className={classNames('session-sidebar-block5', styles['session-sidebar-block5'])} /> : <Trash2 className={classNames('session-sidebar-block5', styles['session-sidebar-block5'])} />}
                      </button>
                    </Tooltip>
                  </Popconfirm>
                </div>
