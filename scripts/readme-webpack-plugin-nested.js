...
const { getModuleList } = require('@kne/modules-dev/lib/utils');
const camelCase = require('@kne/camel-case');

const ReadmeWebpackPlugin_MODULE_NAMES = 'ReadmeWebpackPlugin_MODULE_NAMES';

/** 嵌套路径（Common/Form）转为合法 JS 标识符 */
const toReadmeId = (name) => camelCase(name.replace(/[/\-@]/g, '-'));

class NestedReadmeWebpackPlugin {
  constructor(options) {
  ...
  apply(compiler) {
    ...
    compiler.hooks.beforeCompile.tapPromise('ReadmeWebpackPlugin', async (params) => {
      params[ReadmeWebpackPlugin_MODULE_NAMES] = await (this.options.getModuleList || getModuleList)(
        env.moduleBaseDir,
      );
    });
    compiler.hooks.compilation.tap('ReadmeWebpackPlugin', async (compilation) => {
      ...
      const readmes = `${moduleNames
        .map(
          ({ name }) =>
            `import ${toReadmeId(name)} from '${env.moduleAliasName}/${name}/README.md';`,
        )
        .join('\n')}export default {${moduleNames.map(({ name }) => toReadmeId(name)).join(',')}};`;
...
module.exports = NestedReadmeWebpackPlugin;
